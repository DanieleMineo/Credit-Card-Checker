...
const validateCred = arr => {
  ...
  reversedSum = newArr.slice().reverse().reduce((v1, v2) => v1 + v2); 
  return reversedSum % 10 === 0 ? true : false; 
}

const findInvalidCards = nested => {
  let invalidCards = nested.filter(card =>validateCred(card)===false);
  return invalidCards
  }
 ...
 const idInvalidCardCompanies = invalid => {
   ...
   for (let j =0; j<invalid.length; j++) {
     firstDigit.push(invalid[j][0])
   }
   for (let k = 0; k<firstDigit.length; k++) {
     switch(firstDigit[k]) {
    ...
    companies = companies.filter((ele,pos)=>companies.indexOf(ele) == pos);
    return companies
 }
